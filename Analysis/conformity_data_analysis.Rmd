...
# Analysis -- macht leider noch gar keinen sinn :(
```{r,results='hide'}

<<<<<<< HEAD
priors <- c(set_prior('normal( 0.816 , 0.5 )', class = "b", coef = 'ingroup_norm'),
...
alternative_model <- brm(
  ...
  formula = dilemma_rating ~ dilemma_version + ingroup_norm * both_infos, # regress trials depending on condition
  sample_prior = 'yes', # take samples from prior
  iter = 1000, # set sample size to 10000 to achieve a more precise estimation
...
priors <- prior("normal( 0 , 0.5 )", class = "b") +
          ...
          prior("normal(0.6/0.75 * 1.27 , 0.5)", class = "intercept")

# model_FE = brm(
#   formula = dilemma_rating ~ ingroup_norm + both_infos,
#   data = cleaned_data,
...
self_categorization_model <- brm(
  data = cleaned_data, # define data
  family = sratio,
  formula = bf(dilemma_rating ~ 1 + ingroup_norm + both_infos + ingroup_norm * both_infos), # regress trials depending on condition
  sample_prior = 'yes', # take samples from prior
  iter = 10000, # set sample size to 10000 to achieve a more precise estimation
  prior = priors
  ) #  set the prior to a student_t with 1 degree of freedom, mean = 0 and standard deviation to 100

>>>>>>> be1b1acb811691bcefdcf818a8ec7e38d4b19b09

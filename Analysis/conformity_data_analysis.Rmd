...
list_ingroup_norm <- list(cleaned_data$both_infos)
list_ingroup_norm
list_both_infos <- list(cleaned_data$both_infos)
list_both_infos
list_dilemma_version <- list(cleaned_data$dilemma_version)
list_dilemma_version
list_dilemma_rating <- list(cleaned_data$dilemma_rating)
list_dilemma_rating
cleaned_data
```
...
```{r}
stanfit <- stan(model_code = alternative_prior, pars=c("bOut","bBoth","bIn", "dilemma_version"), iter = 1000, chains = 4,  warmup = 500, seed =123)
# feed the Stan model back into brms
fit <- brm(dilemma_rating ~ dilemma_version + ingroup_norm * both_infos, data = cleaned_data, family = cumulative('logit'), empty = TRUE)
fit$fit <- stanfit
fit <- rename_pars(fit)
summary(fit)
```

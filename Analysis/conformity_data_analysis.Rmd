...
tidy_data <- raw_data %>% 
  ...
  rename(ingroup_norm = ingroup_info_a, daily_dilemma = dilemma_decision1_4, philosophical_dilemma = dilemma_decision2_4, feeling_daily_dilemma = dilemma_decision1_5, feeling_philosophical_dilemma = dilemma_decision2_5) %>%
  ...
  # removes the ingroup_norm variable (information now stored in ingroup_philo and ingroup_daily)
  select(-ingroup_norm) %>%
  ...
  mutate(
    ...
    feeling_daily_dilemma = unique(feeling_daily_dilemma[!is.na(feeling_daily_dilemma)]),
  ...
  unique() %>%
  # creates the ingroup_norm for both daily and philo dilemma according to the coding of the original study: -1 if ingroup prefers active action (e.g. changing the direction of the train) and 1 if ingroup prefers passive action (e.g. not changing the direction of the train)
  mutate(
  ...
  # removes ingroup_norm_daily and ingroup_norm_philo used for creating the one ingroup_norm variable
   select(-ingroup_norm_daily, -ingroup_norm_philo) %>%
  ...
  mutate(
      ...
      feeling_daily_dilemma = as.integer(feeling_daily_dilemma),
...
language_summary <- tidy_data %>% filter(dilemma_version == 'daily_dilemma') %>% 
  summarise(
...
exclusion_summary <- tidy_data %>% filter(dilemma_version == 'daily_dilemma') %>%
  summarise(
...
## Preparing Data for the Exploratory Analysis
```{r}
explor_data <- cleaned_data %>% mutate(
  dilemma_version = case_when(
    dilemma_version == 'philosophical_dilemma' ~ 1,
    dilemma_version == 'daily_dilemma' ~ -1
  ),
  dilemma_version = as.integer(dilemma_version)
)
explor_data
...
## Extending the Models with the Dilemma Version Main-Effect
...
explor_brms_fit_alt <- brm(dilemma_rating ~ dilemma_version + ingroup_norm * both_infos, data = explor_data, family = cumulative('logit'), fit = explor_stanfit_alt, save_all_pars = TRUE)
```
...
explor_brms_fit_sct <- brm(dilemma_rating ~ ingroup_norm * ingroup_agree + both_infos + ingroup_norm * both_infos * outgroup_disagree + dilemma_version, data = explor_data, family = cumulative('logit'), fit = explor_stanfit_sct, save_all_pars = TRUE)
```

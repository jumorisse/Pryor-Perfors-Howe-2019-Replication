...
```{r}
stanc("alternative_model.stan")
```


```{r}
stanfit <- stan(file = alternative_model, data = stan_data_alternative, pars=c("bOut","bBoth","bIn"), iter = 1000, chains = 4,  warmup = 500, seed =123)
# feed the Stan model back into brms
fit <- brm(dilemma_rating ~ ingroup_norm * both_infos, data = cleaned_data, family = cumulative('logit'), empty = TRUE)
fit$fit <- stanfit
fit <- rename_pars(fit)
summary(fit)
```
...
```{r}
stanfit <- stan(model_code = self_categorization_prior, pars=c("bOut","bBoth","bIn", "dilemma_version"), iter = 1000, chains = 4,  warmup = 500, seed =123)
# feed the Stan model back into brms
fit <- brm(dilemma_rating ~ dilemma_version + ingroup_norm * ingroup_agree + both_infos + ingroup_norm * both_infos * outgroup_disagree, data = cleaned_data, family = cumulative('logit'), empty = TRUE)
fit$fit <- stanfit
fit <- rename_pars(fit)
summary(fit)
```

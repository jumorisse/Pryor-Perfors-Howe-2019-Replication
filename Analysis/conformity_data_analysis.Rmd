...
dilemma_rating <- cleaned_data$dilemma_rating
both_infos <- cleaned_data$both_infos
ingroup_norm <- cleaned_data$ingroup_norm
ingroup_agree <- cleaned_data$ingroup_agree
outgroup_disagree <- cleaned_data$outgroup_disagree
...
```{r}
stanfit_alt <- stan(file = alternative_model, data = stan_data_alternative, pars=c("bOut","bBoth","bIn"), iter = 10000, chains = 4,  warmup = 500, seed =123)
# feed the Stan model back into brms
fit_alt <- brm(dilemma_rating ~ ingroup_norm * both_infos, data = cleaned_data, family = cumulative('logit'), empty = TRUE, save_all_pars = TRUE)
...
```{r}
stanfit_sct <- stan(file = sct_model, data = stan_data_sct, pars=c("bOut","bBoth","bIn"), iter = 10000, chains = 4,  warmup = 500, seed =123)
# feed the Stan model back into brms
fit_sct <- brm(dilemma_rating ~ ingroup_norm * ingroup_agree + both_infos + ingroup_norm * both_infos * outgroup_disagree, data = cleaned_data, family = cumulative('logit'), empty = TRUE, save_all_pars = TRUE)
